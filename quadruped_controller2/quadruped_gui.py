...
import rclpy
from quadruped_balance_controller_pid import QuadrupedBalanceController, ControlMode
...
class QuadrupedGUI:
    """Advanced GUI for Quadruped Controller with full feature integration."""
    
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("◢ QUADRUPED NEURAL CONTROL MATRIX ◣")
    ...
    def create_widgets(self):
        ...
        title_label = tk.Label(header_frame, text="QUADRUPED NEURAL CONTROL MATRIX", 
    ...
    def create_control_panel(self, parent):
        container = tk.Frame(parent, bg=CyberpunkStyle.BG_DARK, width=450)
        ...
        # --- STANCE & GAIT ---
        stance_gait_frame = CyberpunkFrame(scrollable_frame, "STANCE & GAIT", CyberpunkStyle.ACCENT_RED)
        stance_gait_frame.pack(fill='x', pady=5, padx=5)
        
        self.stance_var = tk.StringVar(value="stand")
        stances = [("STAND", "stand"), ("CROUCH", "crouch"), ("WIDE", "wide"), ("NARROW", "narrow")]
        stance_btn_frame = tk.Frame(stance_gait_frame, bg=CyberpunkStyle.BG_PANEL)
        ...
        for text, value in stances:
            btn = tk.Radiobutton(stance_btn_frame, text=f"◢ {text} ◣", variable=self.stance_var,
                               ...
                               selectcolor=CyberpunkStyle.ACCENT_RED, font=CyberpunkStyle.FONT_MAIN)
            btn.pack(side='left', padx=2, fill='x', expand=True)

        self.gait_enabled_var = tk.BooleanVar(value=False)
        gait_check = tk.Checkbutton(stance_gait_frame, text="◢ ENABLE GAIT ◣", variable=self.gait_enabled_var,
                                  command=self.toggle_gait, bg=CyberpunkStyle.BG_PANEL, fg=CyberpunkStyle.TEXT_PRIMARY,
                                  selectcolor=CyberpunkStyle.BG_WIDGET, font=CyberpunkStyle.FONT_MAIN)
        gait_check.pack(pady=5, fill='x')
        self.gait_freq_scale = CyberpunkScale(stance_gait_frame, "Frequency (Hz):", from_=0.1, to=3.0, resolution=0.1,
                                            initial_value=1.0, accent_color=CyberpunkStyle.ACCENT_RED)
        self.gait_height_scale = CyberpunkScale(stance_gait_frame, "Step Height (°):", from_=5.0, to=40.0, resolution=1.0,
                                              initial_value=15.0, accent_color=CyberpunkStyle.ACCENT_RED)
        self.gait_freq_scale.pack(fill='x', padx=5)
        self.gait_height_scale.pack(fill='x', padx=5)
        
        gait_apply_btn = tk.Button(stance_gait_frame, text="◢ APPLY GAIT PARAMS ◣", command=self.toggle_gait,
                                  bg=CyberpunkStyle.BG_WIDGET, fg=CyberpunkStyle.ACCENT_RED, relief='flat')
        gait_apply_btn.pack(pady=5, fill='x', padx=5)
        
        # --- MANUAL OVERRIDE ---
        manual_frame = CyberpunkFrame(scrollable_frame, "MANUAL OVERRIDE", CyberpunkStyle.ACCENT_YELLOW)
    ...
    def create_leg_control_tab(self, notebook):
        ...
        for i, leg_name in enumerate(['FL', 'FR', 'RL', 'RR']):
            leg_frame = CyberpunkFrame(leg_tab_frame, f"{leg_name} Leg", CyberpunkStyle.ACCENT_YELLOW)
            leg_frame.grid(row=i//2, column=i%2, padx=5, pady=5, sticky='nsew')
            ...
            set_btn = tk.Button(leg_frame, text=f"◢ SET {leg_name} ◣", 
                                ...
                                bg=CyberpunkStyle.BG_WIDGET, fg=CyberpunkStyle.ACCENT_YELLOW, relief='flat')
    ...
    def create_monitor_panel(self, parent):
        ...
        top_row = tk.Frame(monitor_frame, bg=CyberpunkStyle.BG_PANEL)
        top_row.pack(fill='x')
        
        status_frame = CyberpunkFrame(top_row, "SYSTEM STATUS", CyberpunkStyle.ACCENT_CYAN)
        status_frame.pack(fill='x', pady=5, padx=5, side='left', expand=True)
        
        status_grid = tk.Frame(status_frame, bg=CyberpunkStyle.BG_PANEL)
        status_grid.pack(fill='x', pady=5)
        
        self.status_labels = {}
        status_items = [("ROLL", "roll", "°"), ("PITCH", "pitch", "°"), ("STABILITY", "stability", "%"),
                        ("FALL RISK", "fall_risk", ""), ("CORE TEMP", "temperature", "°C"), ("MODE", "control_mode", "")]
        for i, (label, key, unit) in enumerate(status_items):
            tk.Label(status_grid, text=f"{label}:", bg=CyberpunkStyle.BG_PANEL, fg=CyberpunkStyle.TEXT_SECONDARY,
                     font=CyberpunkStyle.FONT_MAIN, anchor='w').grid(row=i, column=0, sticky='w', padx=5, pady=2)
            val_widget = tk.Label(status_grid, text="--", bg=CyberpunkStyle.BG_PANEL, font=CyberpunkStyle.FONT_MONO, anchor='e')
            val_widget.grid(row=i, column=1, sticky='e', padx=5, pady=2)
            self.status_labels[key] = (val_widget, unit)
        
        leg_status_frame = CyberpunkFrame(top_row, "LEG CONTROL STATUS", CyberpunkStyle.ACCENT_YELLOW)
        leg_status_frame.pack(fill='both', pady=5, padx=5, side='right', expand=True)

        self.leg_status_labels = {}
        for i, leg in enumerate(['FL', 'FR', 'RL', 'RR']):
            row_frame = tk.Frame(leg_status_frame, bg=CyberpunkStyle.BG_PANEL)
            row_frame.pack(fill='x')
            tk.Label(row_frame, text=f"{leg}:", bg=CyberpunkStyle.BG_PANEL, fg=CyberpunkStyle.TEXT_SECONDARY,
                     font=CyberpunkStyle.FONT_MAIN).pack(side='left')
            label = tk.Label(row_frame, text="AUTO", bg=CyberpunkStyle.BG_PANEL, fg=CyberpunkStyle.ACCENT_GREEN,
                             font=CyberpunkStyle.FONT_MONO)
            label.pack(side='right')
            self.leg_status_labels[leg] = label
        ...
        self.fig, (self.ax1, self.ax2) = plt.subplots(2, 1, figsize=(10, 8), facecolor=CyberpunkStyle.BG_PANEL)
        self.fig.tight_layout(pad=3.0)
        
        self.roll_line, = self.ax1.plot(self.time_data, self.roll_data, color=CyberpunkStyle.ACCENT_BLUE, linewidth=2)
        self.pitch_line, = self.ax2.plot(self.time_data, self.pitch_data, color=CyberpunkStyle.ACCENT_ORANGE, linewidth=2)
        ...
        for ax, color, title in [(self.ax1, CyberpunkStyle.ACCENT_BLUE, "ROLL AXIS"),
            ...
            for spine in ax.spines.values(): spine.set_color(CyberpunkStyle.BORDER_INACTIVE)
    ...
    def toggle_gait(self, _=None):
        self.controller.enable_gait(self.gait_enabled_var.get(), self.gait_freq_scale.get(), self.gait_height_scale.get())
    ...
    def update_gui_elements(self, status):
        try:
            ...
            for leg_name, leg_data in status['legs'].items():
                mode = leg_data['mode'].replace('_', ' ').upper()
                color = CyberpunkStyle.ACCENT_YELLOW if 'MANUAL' in mode else CyberpunkStyle.ACCENT_GREEN
                self.leg_status_labels[leg_name].config(text=mode, fg=color)

            self.balance_enabled_var.set(status['control']['balance_enabled'])
            self.control_mode_var.set(status['control']['mode'])
            self.gait_enabled_var.set(status['control']['gait_enabled'])
